...
| M3 ⚡ | Fishing feel slice (gray-box) | M0, M2 | in-progress — `claude/sushi-m4-cook` (fishing files touched here too, see note). Catch zone reworked 2026-08-06 (Giahy): now moves (Stardew Valley-style, deterministic sine-sum drift) instead of a static band — `FishingCatch.targetCenterAt`/`isInBand`, `FishingConfig.TARGET_*`. Studio-unverified since the rework; needs another playtest pass before the feel gate. |
