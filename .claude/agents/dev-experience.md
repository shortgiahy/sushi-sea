...
- **Memory (mem0 MCP):** at task start, `search_memories` with `user_id: "giahy"` for prior decisions on your area before reading code. At task end, `add_memory` (`user_id: "giahy"`, `metadata.type: "project"`) for any durable decision *and its why* — not task status, not code facts derivable from the repo. Tools: `add_memory` · `search_memories` · `get_memories`. If they are unavailable, say so in your report and fall back to `BUILD_LOG.md`; never silently skip the search.
